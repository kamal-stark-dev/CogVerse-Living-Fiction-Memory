...
export async function fetchUniverseLedger(universe) {
  const res = await fetch(
    `${BASE_URL}/universes/${encodeURIComponent(universe)}/ledger`,
  );
  if (!res.ok) throw new Error("Failed to load universe ledger");
