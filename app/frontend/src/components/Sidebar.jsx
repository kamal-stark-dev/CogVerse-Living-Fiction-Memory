...
function accentFor(name, list) {
  const idx = list.indexOf(name) % ACCENTS.length
  return ACCENTS[idx] ?? ACCENTS[0]
}

export default function Sidebar({ selectedUniverse, selectedCharacter, onSelectCharacter }) {
  ...
  return (
    <aside className="archive">
      ...
      {error && <p className="empty-note">Couldn't reach the backend: {error}</p>}

      <div className="archive-list">
        {universes.map((universe) => {
          const accent = accentFor(universe, universes)
              ...
              {isOpen && (
                <div className="character-list">
                  {(charactersByUniverse[universe] || []).map((char) => (
                    <button
                      ...
                      className={`character-row ${
                        selectedCharacter === char && selectedUniverse === universe ? 'active' : ''
                      }`}
                      style={{ '--u-accent': accent }}
                  ...
                  {charactersByUniverse[universe]?.length === 0 && (
                    <span className="empty-note">No characters found in this universe's folder</span>
                  )}
                </div>
              )}
            </div>
        ...
        {!error && universes.length === 0 && (
          <p className="empty-note">
            No universes found. Check that REPO_DATA_DIR on the backend points at your data/ folder.
          </p>
        )}
      </div>
    </aside>
