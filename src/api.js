...
export async function fetchLoreLedger(universe) {
  const res = await fetch(`${BASE_URL}/universes/${encodeURIComponent(universe)}/lore`)
  if (!res.ok) throw new Error('Failed to load lore ledger')
