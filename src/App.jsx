...
import LoreLedger from "./components/LoreLedger";
import CrossUniverseGateway from "./components/CrossUniverseGateway";
import { sendChat, fetchUniverses } from "./api";
...
export default function App() {
  ...
  useEffect(() => {
    fetchUniverses()
      .then(setAllUniverses)
      .catch((err) => console.error("Failed to fetch universes:", err));
  }, []);

  const handleReferenceFact = (factName) => {
    setCrossUniverse(true);
    setReferenceUniverse(selectedUniverse || "");
    setReferenceQuery(factName);
  };
  ...
  return (
    <div className="app-shell">
      ...
      {ledgerOpen && selectedUniverse && (
        <LoreLedger
          universe={selectedUniverse}
          onReferenceFact={handleReferenceFact}
